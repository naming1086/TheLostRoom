    ...
    public class AudioStreamDevicesChangedNotify : MonoBehaviour
	{
        ...
        void Update()
        {
            if (this.outputDevicesChanged)
            {
                ...
                AudioStreamSupport.LOG(LogLevel.INFO, this.logLevel, this.gameObjectName, this.OnError, "Received DEVICELISTCHANGED | DEVICELOST | RECORDLISTCHANGED notification");

                var d = FMODSystemsManager.AvailableOutputs(this.logLevel, this.gameObjectName, this.OnError);
                if (d[0].guid != this.notification_system.forOutpuDevice.guid)
                {
                    // need to stop everybody @ system 0 in order to properly release it
                    var asods0 = FindObjectsOfType<AudioSourceOutputDevice>();
                    foreach( var asod0 in asods0)
                        if (asod0.outputDriverID == 0)
                        {
                            asod0.StopFMODSound();
                        }
                    ...
                    // install new system on 0
                    this.CreateNotificationSystem();

                    // restart all prior 0 holders
                    foreach (var asod0 in asods0)
                        if (asod0.outputDriverID == 0)
                        {
                            asod0.StartFMODSound();
                        }
                }
