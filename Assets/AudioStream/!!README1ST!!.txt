...
[ you will also get error about importing multichannel audio file used for demo purposes:
...
Required Unity package is available primarily on FMOD's download site: https://www.fmod.com/download

Please note there is also (almost always) identical version of FMOD Unity Integration available on the Asset Store ( https://assetstore.unity.com/packages/tools/audio/fmod-for-unity-161631 ),
but it's not always up to date with latest official release and might differ significantly to a degree that asset won't be compatible with it.
So I currently recommend downloading and using Unity Integration from FMOD official site.

The asset currently (0920202) supports FMOD 2.01.04 and up.
...
> AudioStream uses only low level API of FMOD Studio and only really requires a part of the "Plugins" folder from the FMOD package.
...
* 'Plugins/FMOD/src/Runtime/' folder (Timeline support can be omitted, too)
