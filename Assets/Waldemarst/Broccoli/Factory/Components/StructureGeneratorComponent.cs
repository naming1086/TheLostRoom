	...
	public class StructureGeneratorComponent : TreeFactoryComponent, IStructureGeneratorComponent {
		...
		public override bool Process (TreeFactory treeFactory, 
			...
			if (processControl != null) {
				if (pipelineElement != null && tree != null && processControl.pass == 1 && !useLocalCache) {
		...
		public void CommitBranchCurve (int branchId, int nodeIndex, Vector3 offset, bool commitToFollowup = false) {
			for (int i = 0; i < structureGeneratorElement.flatStructures.Count; i++) {
				if (structureGeneratorElement.flatStructures[i].id == branchId) {
		...
		public void UnlockStructure (int branchId) {
			for (int i = 0; i < structureGeneratorElement.flatStructures.Count; i++) {
				if (structureGeneratorElement.flatStructures[i].id == branchId) {
