...
namespace AudioStream
{
    ...
    public static partial class FMODSystemsManager
    {
            ...
            )
        {
            ...
            if (fmodsystem.forOutpuDevice.id == 0)
            {
                if (FMODSystemsManager.system_output0_refCount < 1)
                    AudioStreamSupport.LOG(LogLevel.WARNING, logLevel, gameObjectName, onError, "System {0} for output {1} is being overreleased", fmodsystem.System.handle, fmodsystem.forOutpuDevice.id);
            ...
            if (releaseAndRemoveSystem)
            {
                ...
                if (!FMODSystemsManager.systems4devices.ContainsValue(fmodsystem))
                    AudioStreamSupport.LOG(LogLevel.ERROR, logLevel, gameObjectName, onError, "System {0} for output device {1} being released was not previously created via FMODSystemsManager", fmodsystem.System.handle, fmodsystem.forOutpuDevice.id);
                else
                    FMODSystemsManager.systems4devices.Remove(fmodsystem.forOutpuDevice.id);

                AudioStreamSupport.LOG(LogLevel.INFO, logLevel, gameObjectName, onError, "Released {0} system {1} for output driver {2}", fmodsystem.isNotificationSystem ? "notification" : "", fmodsystem.System.handle, fmodsystem.forOutpuDevice.id);
