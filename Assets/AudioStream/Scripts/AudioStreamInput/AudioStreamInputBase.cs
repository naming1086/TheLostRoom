...
namespace AudioStream
{
    public abstract class AudioStreamInputBase : MonoBehaviour
    {
        ...
        [Header("[Advanced]")]
        [Tooltip("FMOD defaults to the 'correct'/optimal DSP buffer size (lenght and no. of buffes) for given input, so it's recommended to leave this to default (on)\r\nWhen overriden (off), DSP buffer lenght and no. of buffers can be set manually to drive latency as low as possible (usually in 'until things still work' way)." +
            "\r\nIf you use more than one AudioStreamInput* component, any custom DSP settings are taken only from one - the one started first - since a single FMOD system is used for all input sounds and DSP buffers are set on it." +
            "\r\nNote that this also means that it's not possible to change DSP buffer settings at runtime / once FMOD system had been created." +
            "\r\n\r\nPlease see also documentation for further platform specific recording notes.")]
        public bool useAutomaticDSPBufferSize = true;
        [Tooltip("Set DSP buffer size as small as possible for small latency.\r\nYou can go very low on platforms which support it - such as 32/2 on desktops - but you have to find correct *combination* with which the FMOD mixer will still work with current input and drivers.")]
        [SerializeField]
        ...
        /// </summary>
        protected FMODSystemInputDevice recording_system;
        /// <summary>
        ...
        protected virtual IEnumerator Start()
        {
            ...
            // create/get enum+record system
            this.recording_system = FMODSystemsManager.FMODSystemInputDevice_Create(
                this.useAutomaticDSPBufferSize ? 0 : this.dspBufferLength_Custom
                , this.useAutomaticDSPBufferSize ? 0 : this.dspBufferCount_Custom
                , this.logLevel
                , this.gameObjectName
                , this.OnError
        ...
        public IEnumerator Record_CR()
        {
            ...
            result = this.recording_system.system.getRecordDriverInfo(this.recordDeviceId, out name, namelen, out guid, out recRate, out speakermode, out recChannels, out driverstate);
            ...
            result = this.recording_system.system.createSound(string.Empty, FMOD.MODE.LOOP_NORMAL | FMOD.MODE.OPENUSER, ref exinfo, out sound);
            ERRCHECK(result, "this.recording_system.system.createSound");

            LOG(LogLevel.INFO, "Opened device {0}, channels: {1}, format: {2}, rate: {3} (sound length: {4}, info size: {5}))", this.recordDeviceId, exinfo.numchannels, exinfo.format, exinfo.defaultfrequency, exinfo.length, exinfo.cbsize);

            result = this.recording_system.system.recordStart(this.recordDeviceId, sound, true);
        ...
        {
            result = this.recording_system.system.update();
            ...
            ERRCHECK(result, "this.recording_system.system.getRecordPosition");
        ...
        protected virtual void OnDestroy()
        {
            ...
            /*
                Shut down
            */
            FMODSystemsManager.FMODSystemInputDevice_Release(this.recording_system, this.logLevel, this.gameObjectName, this.OnError);
        }
        #endregion
