...
namespace AudioStream
{
    ...
    public class FMODSystemOutputDevice
    {
        ...
        /// <summary>
        /// Id this system for available/notification updates
        /// </summary>
        public readonly FMODSystemsManager.OUTPUT_DEVICE forOutpuDevice;
        /// <summary>
            ...
            )
        {
            ...
            */
            ...
            this.forOutpuDevice.id = forOutputDevice;
            this.forOutpuDevice.name = od_name;
            this.forOutpuDevice.guid = od_guid;
            this.forOutpuDevice.samplerate = od_systemrate;
            this.forOutpuDevice.speakermode = od_speakermode;
            this.forOutpuDevice.channels = od_speakermodechannels;
        }
        /// <summary>
            ...
            )
        {
                ...
                && this.outputDevicesChangedCallback == null)
            {
                ...
                AudioStreamSupport.LOG(LogLevel.INFO, LogLevel.INFO, gameObjectName, onError, "Installed DEVICELISTCHANGED | DEVICELOST | RECORDLISTCHANGED callback on driver {0} ", this.forOutpuDevice.id);
            ...
            )
        {
            if (this.SystemHandle != global::System.IntPtr.Zero)
            {
                ...
                AudioStreamSupport.LOG(LogLevel.DEBUG, logLevel, gameObjectName, onError, "Released system {0} for output {1}", this.System.handle, this.forOutpuDevice.id);
            ...
            )
        {
            ...
            AudioStreamSupport.LOG(LogLevel.DEBUG, logLevel, gameObjectName, onError, "Created sounds {0} on system {1} / output {2}", sound.handle, System.handle, this.forOutpuDevice.id);
            ...
            )
        {
            ...
            if (sound.hasHandle() && this.sounds.ContainsKey(sound))
            {
                ...
                AudioStreamSupport.LOG(LogLevel.DEBUG, logLevel, gameObjectName, onError, "Released sound {0} for system {1}, output {2}", sound.handle, SystemHandle, this.forOutpuDevice.id);
            ...
            {
                AudioStreamSupport.LOG(LogLevel.WARNING, LogLevel.WARNING, gameObjectName, onError, "Not releasing sound {0} for system {1}, output {2}, {3} in list", sound.handle, SystemHandle, this.forOutpuDevice.id, this.sounds.ContainsKey(sound) ? "" : "NOT");
            }
            ...
            )
        {
            ...
            result = System.createSound(audioUri
                    ...
                    , ref exinfo
                    , out sound);
            AudioStreamSupport.ERRCHECK(result, logLevel, gameObjectName, onError, string.Format("System.createSound {0}", audioUri), false);
            ...
            )
        {
            ...
            FMOD.ChannelGroup master;
            result = System.getMasterChannelGroup(out master);
            AudioStreamSupport.ERRCHECK(result, logLevel, gameObjectName, onError, "System.getMasterChannelGroup");

            var loopcount = loop ? -1 : 0;
            result = onSound.setLoopCount(loopcount);
            AudioStreamSupport.ERRCHECK(result, logLevel, gameObjectName, onError, "onSound.setLoopCount");

            // (log verification)
            result = onSound.getLoopCount(out loopcount);
            AudioStreamSupport.ERRCHECK(result, logLevel, gameObjectName, onError, "onSound.getLoopCount");


            result = System.playSound(onSound, master, !startImmediately, out channel);
            AudioStreamSupport.ERRCHECK(result, logLevel, gameObjectName, onError, "System.playSound");

            result = channel.setVolume(volume);
            AudioStreamSupport.ERRCHECK(result, logLevel, gameObjectName, onError, "channel.setVolume");

            if (withMixmatrix != null)
            {
                result = this.SetMixMatrix(channel, withMixmatrix, outchannels, inchannels);
                AudioStreamSupport.ERRCHECK(result, logLevel, gameObjectName, onError, "SetMixMatrix");
            }
